...
SETS = ("Waters — Lists", [
    ...
    ("Name the five oceans", 0, [
        ("Pacific", ["Pacific Ocean"]),
        ("Atlantic", ["Atlantic Ocean"]),
        ("Indian", ["Indian Ocean"]),
        ("Arctic", ["Arctic Ocean"]),
        ("Southern", ["Southern Ocean", "Antarctic", "Antarctic Ocean"]),
    ], "The Southern Ocean, ringing Antarctica, is the newest — widely "
       "recognized only since 2000."),
    ("Name three of the four seas touching Turkey", 3, [
        ("Black Sea", []),
        ("Aegean Sea", ["Aegean"]),
        ("Mediterranean Sea", ["Mediterranean"]),
        ("Sea of Marmara", ["Marmara"]),
    ], "The Marmara is the small fourth, entirely inside Turkey between "
...
def build_sets():
    name, cards = SETS
    blocks = []
    for question, quota, items, note in cards:
        card = f"{question}\n---\n"
        if quota:
            card += f"quota: {quota}\n"
        for text, variants in items:
            card += f"+ {text}\n"
            for v in variants:
                card += f"= {v}\n"
        if note:
